...
#utilizzo la colonna selling price perchè è quella su cui vogliamo fare la previsione
#facendo la summary si nota che 
```{r regressione lineare multipla}
train_lm<-lm(log(selling_price) ~.,data = train_set)
summary(train_lm)

```

#cerco di aumenare l'R2 eliminando qualche colonna del training set in modo tale da mantenere solo le colonne che migliorano il modello ma anche eliminando le colonne meno significative l'R2 migliora cosi poco da poterle definire come irrilevanti da eliminare
```{r eliminazione di alcune colonne per migliorare l'R2}
train_lm1<-lm(log(selling_price) ~.-year,data = train_set)
summary(train_lm1) #si abbassa l'R2 quindi lasciare year

train_lm2<-lm(log(selling_price) ~.-km_driven ,data = train_set)
summary(train_lm2)#si abbassa di poco

train_lm3<-lm(log(selling_price) ~.-engine_CC ,data = train_set)
summary(train_lm3)# si abbassa

train_lm4<-lm(log(selling_price) ~.-max_power_bhp ,data = train_set)
summary(train_lm4)# si abbassa

train_lm5<-lm(log(selling_price) ~.-transmission ,data = train_set)
summary(train_lm5)# si abbassa

train_lm6<-lm(log(selling_price) ~.-fuelLPG -seats -fuelCNG,data = train_set)
summary(train_lm6)# aumenta di pochissimo

train_lm7<-lm(log(selling_price) ~.-year -max_power_bhp - engine_CC,data = train_set)
summary(train_lm7)# si abbassa tantissimo per cui prenderei queste tre colonne come migliori

train_lm8<-lm(log(selling_price) ~.-km_driven -transmission -fuelPetrol,data = train_set)
summary(train_lm8)# è sempre alto l'R2 ma non c'è cosi tanta differenza con quello di partenza, però cosi ottengo dei valori di t test abbastanza positivi

```

#considero più modelli e li confronto utilizzando ANOVA. Adotto anche questa tecnica per dimostrare che i due modelli non hanno praticamente nulla di diverso e possono essere considerati praticamente come uguali e identicamente paragonabili anche sulla base dell'RSS
```{r confronto di più modelli tramite ANOVA}
anova(train_lm,train_lm6)

```


### stabilire ipotesi nulla e ipotesi alternativa
### analisi dei residui
```{r calcolo dei residui}
...
#######  da usare solo se serve ma non utile per ora
