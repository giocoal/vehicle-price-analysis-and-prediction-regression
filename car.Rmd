...
Sono presenti 1183 righe che descrivono auto uguali, dello stesso anno, con gli stessi km e prezzo. Inoltre queste righe non possono riferirsi alla stessa auto rivenduta più volte altrimenti cambierebbe il campo `owner`. Questa cosa è strana e probabilmente per la maggior parte delle righe è dovuto ad un cattivo inserimento dei dati, per questo decidiamo di rimuoverle.
...
Individuo le marche con un numero di conteggi minore di 13, su cui non potrei fare una corretta statistica inferenziale.

```{r conteggi per categorie make}
conteggi <- count(car_noduplicati, make, sort = TRUE)
nomake<-conteggi[conteggi$n < 13,]
nomake
```
...
car_noduplicati_nomake <- droplevels(car_noduplicati[!(car_noduplicati$make %in% nomake$make),])

str(car_noduplicati_nomake$make)
```
...
car_dummy <- car_noduplicati_nomake[,car_nums_colnames_log]
...
car_dummy <- cbind(car_dummy,transmission = car_noduplicati_nomake$transmission)
...
dummy_temp <- data.frame(model.matrix( ~make, data = car_noduplicati_nomake))[,-1]
car_dummy <-cbind(car_dummy,dummy_temp)

#converto la variabile fattoriale 'fuel' in dummy e la aggiungo a car_dummy:
dummy_temp <- data.frame(model.matrix( ~fuel, data = car_noduplicati_nomake))[,-1]
car_dummy <-cbind(car_dummy,dummy_temp)

#converto la variabile fattoriale 'seller_type' in dummy e la aggiungo a car_dummy:
dummy_temp <- data.frame(model.matrix( ~seller_type, data = car_noduplicati_nomake))[,-1]
car_dummy <-cbind(car_dummy,dummy_temp)

#converto la variabile fattoriale 'owner' in dummy e la aggiungo a car_dummy:
dummy_temp <- data.frame(model.matrix( ~owner, data = car_noduplicati_nomake))[,-1]
car_dummy <-cbind(car_dummy,dummy_temp)
...
Per poter eseguire la regressione lineare abbiamo diverse assunzioni da fare: DA TRADURRE
...
Come primo passo realizzo un modello di regressione, sul `train_set` in cui considero tutte le 30 variabili
...
Ciò che risulta è un modello con un valore dell'R quadro aggiustato elevato `Adjusted R-squared: 0.8516`. Il problema di questo modello però è che molti degli attributi non sono significativi, quindi probabilmente il valore R alto è dovuto ad un fenomeno di overfitting. Questa cosa la verificheremo poi nella sezione successiva in cui testeremo i risultati sul test set.
...
1.  Linearità delle relazioni tra variabile dipendente e le variabili indipendenti

2.  Residui distribuiti in modo normale

3.  Omoschedasticitá (varianza costante per i residui)

4.  Indipendenza dei residui

    DESCRIVERE BENE GRAFICI (cercare il punto 5496)

Da questi grafici si vede che effettivamente ci sono dei problemi nel modello, in particolare nel `Normal Q-Q` si vede gli `Strandardized residual` a sinistra (con `Theoretical Quantiles` tra -4 e -2) siano più bassi del valore teorico. Anche nel grafico della `Scale-Location` si vedono delle anomalie
...
Come ci aspettavamo l'attributo `log_max_power_bhp` è molto significativo per la descrizione del modello con un p-value molto basso e un R quadro comunque abbastanza elevato `Adjusted R-squared:  0.4672.` Vediamo ora qui di seguito la rappresentazione grafica della retta trovata.
...
Il processo di stepAIC ha portato all'esclusione di 12 attributi. Questo ha portato il valore dell'R quadro aggiustato a `Adjusted R-squared:  0.8525`, cioè identico a quello iniziale. Togliendo quelle colonne abbiamo quindi alleggerito il modello senza perdere di performance.
...
Risultano quindi esclusi dal modello 21 degli attributi iniziali, il modello è ora composto da 22 attributi molto significativi. Il valore dell'R quadro aggiustato é `Adjusted R-squared:  0.851`, cioè praticamente identico a quello individuato in precedenza.
