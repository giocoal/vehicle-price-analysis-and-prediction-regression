...
Come ultima verifica del dataset individuo se sono presenti righe uguali.

```{r numero righe uguali}
sum(duplicated(car))
```

Questo significa che sono presenti più volte gli stessi modelli di auto con esattamente lo stesso prezzo, anni di vita e numero di km. Questa cosa però può essere giustificata dal fatto che i prezzi ed i km vengono molto spesso arrotondati in modo aggressivo, decidiamo dunque di lasciare tutte le righe.
...
#  hist(log10(car_num[[i]]), xlab = i, main = '')
#}
#par(mfrow=c(1,1))
...
Sono presenti 1183 righe che descrivono auto uguali, dello stesso anno, con gli stessi km e prezzo. La cosa é sospetta ed inoltre queste righe non possono riferirsi alla stessa auto rivenduta più volte altrimenti cambierebbe il campo `owner`. Probabilmente per la maggior parte delle righe è dovuto ad un cattivo inserimento dei dati, per questo decidiamo di rimuoverle.

```{r rimuovo righe ripetute e grafico distribuzione selling_price}
car_noduplicati <- distinct(car)

car %>% ggplot()+geom_histogram(aes(log_selling_price, ..density..), bins = 30, )+geom_density(aes(log_selling_price))+ggtitle('Istogramma del logaritmo di selling_price con righe duplicate')+xlab('Selling Price')+ylab('Densità')

car_noduplicati %>% ggplot()+geom_histogram(aes(log_selling_price, ..density..), bins = 30, )+geom_density(aes(log_selling_price))+ggtitle('Istogramma del logaritmo di selling_price senza righe ripeture')+xlab('Selling Price')+ylab('Densità')

par(pty="s")
qqnorm(car$log_selling_price, main = "Normal Q-Q Plot con righe duplicate")
qqline(car$log_selling_price)

par(pty="s")
qqnorm(car_noduplicati$log_selling_price, main = "Normal Q-Q Plot senza righe duplicate")
qqline(car_noduplicati$log_selling_price)
```

Rimuovendo le righe duplicate si vede anche un miglioramento nella distribuzione della variabile `log_selling_price`. Anche il Normal Q-Q Plot é migliorato anche se le code risultano comunque un pó piú spesse della distribuzione normale.

### Categorie poco rappresentate

Individuo le marche con un numero di conteggi minore di 13, su cui non potrei fare una corretta statistica inferenziale una volta splittato il dataset in training e test.
...
Individuo gli owner con un numero di conteggi minore di 13, su cui non potrei fare una corretta statistica inferenziale una volta splittato il dataset in training e test sets.
...
Creo un nuovo dataset chiamato `car_dummy` al quale aggiungo le variabili quantitative in forma logaritmica dove necessario. Al dataset appena creato aggiungo le colonne relative alle variabili categoriche. Per ogni attributo vengono generate n-1 nuove colonne dove n è il numero di modalità di quel particolare attributo.
...
Con tutti i valori a zero per una classe di attiributi si ottiene: `Audi` per la classe `make`, `First Owner` per la classe `owner`, `Automatic` per la classe `transmission`, `Diesel` per la classe `fuel`, `Individual` per la classe `seller_type`.
...
Per poter eseguire la regressione lineare abbiamo diverse assunzioni da fare:

-   **Linearitá** tra la variabile target e i regressori

-   **Omoschedasticitá**: la varianza dei residui sia omogenea

-   **Indipendenza** tra le diverse osservazioni

-   **Normalitá** della distribuzione dei residui

-   Bassa **multicollinearitá**

Andremo a verificare se queste assunzioni sono verificate per ogni modello.

Iniziamo quindi realizzando un modello di regressione sul `train_set` in cui considero tutte le variabili.
...
Ciò che risulta è un modello con un valore dell'R quadro aggiustato elevato `Adjusted R-squared: 0.8587`. Il problema di questo modello però è che diversi attributi sono poco significativi quindi probabilmente il valore R alto è dovuto ad un fenomeno di overfitting. Questa cosa la verificheremo poi nella sezione successiva in cui testeremo i risultati sul test set. Inoltre alcuni degli attributi presenti sono correlati tra loro e dovremo quindi procedere a rimuoverli.

Dalla tabella dei coefficienti si può osservare anche che gli attributi dummy che presentano la significatività maggiore sono quelli corrispondenti alle modalitá piú diverse dalla "modalitá zero", quella che si ottiene con tutte le colonne di una certa classe uguali a zero (es. BMW e Mercedes sono le piú simili alle Audi).

Studiamo ora residui del modello generando i diagnostic plots.

```{r plot modello completo, warning=FALSE}
par(pty="s")
...
1.  Il primo mostra eventuali pattern non lineari nei residui che potrebbero suggerire una relazione non lineare tra target e regressori. Nel nostro caso risulta come ci si aspettava una retta orizzontale.

2.  Il secondo verifica se la distribuzione dei residui é Gaussiana. Nel nostro caso per valori dei quantili teorici tra -2 e -4 c'é un allontanamento dei dati dalla retta, questo mostra un'asimmetria della distribuzione che affronteremo in seguito.

3.  Il terzo verifica l'assunzione dell'omoschedasticitá. Nel nostro caso si ottiene con buona approssimazione una retta orizzontale che verifica l'assunzione.

4.  Il quarto permette di trovare eventuali outliers rimasti che potrebbero modificare in modo significativo il modello. Dei nostri punti nessuno si trova oltre la Cook's distance; il più vicino è il punto 5491 che, dato che compare anche negli altri grafici, decidiamo di rimuoverlo.

```{r rimuovo osservazione outlier}
train_set <- subset(train_set, !row.names(train_set) == 5491)
```

### Feature selection

Cerchiamo ora di ridurre il numero di attributi necessari per aumentare l'interpretabilità e tentare di ridurre i problemi presenti. Per prima cosa andiamo a rimuovere dal dataset gli attributi che presentano problemi di collinearitá, facciamo riferimento alla tabella delle correlazioni della sezione "Analisi delle correlazioni". Dalla tabella si puó vedere che l'attributo `log_engine_CC` é molto correlato con `log_max_power_bhp` e decidiamo dunque di rimuoverlo.
...
Dalla tabella delle correlazioni si puó inoltre notare come `log_max_power_bhp` e `log_age` siano i due attributi più correlati con `log_selling_price` rispettivamente con valore di `0.74` e `0.67`. Scegliamo quindi `log_max_power_bhp` come primo attributo con cui costruire il nuovo modello.
...
Come ci aspettavamo l'attributo `log_max_power_bhp` è molto significativo per la descrizione del modello con un p-value molto basso e un R quadro comunque abbastanza elevato `Adjusted R-squared:  0.4451`. Vediamo ora qui di seguito la rappresentazione grafica della retta trovata.
...
I grafici risultano molto simili a quelli del modello completo presentando lo stesso problema nel Normal Q-Q.

Per rimuovere altri attributi é possibile procedere in due modi:

-   selezionare una diversa "modalitá nulla"
...
Risultano quindi esclusi dal modello 8 degli attributi iniziali, il modello è ora composto da 23 attributi molto significativi. Il valore dell'R quadro aggiustato é `Adjusted R-squared:  0.8568`, cioè praticamente identico a quello individuato in precedenza.

Anche in questo caso andiamo a mostrare i grafici dei residui del modello trovato.

```{r plot modello stepAIC con parametro k, warning=FALSE}
par(pty="s")
plot(modello_stepAIC_k)
```

I grafici dei residui anche in questo caso risultano abbastanza buoni. La forma anomala del Normal Q-Q indica una asimmetria della distribuzione; questo é probabilmente dovuto alla non perfetta normalità delle variabili considerate.

Nonostante il modello descriva bene i dati potrebbe essere molto migliore se ci fosse una variabile che considera lo stato dell'auto, questo infatti é uno dei parametri che maggiormente puó influenzare il costo di un'auto usata.

Utilizzando ora la funzione `anova` confrontiamo il modello ottenuto con la funzione stepAIC e quello ottenuto dal processo di stepAIC con parametro k per vedere se la rimozione degli attributi meno significativi con l'uso del parametro k è giustificata.

```{r confronto modello_completo con modello_stepAIC_k}
anova(modello_completo, modello_stepAIC_k)
```
...
###verificare se é accettabile quel valore di F #da verificare se tutto corretto: La funzione anova prenderà gli oggetti del modello come argomenti e restituirà un ANOVA che verifica se il modello più complesso (modificato dallo stepAIC k) è significativamente migliore nell'acquisizione dei dati rispetto al modello più semplice (modello con lo stepAIC). Se il p-value risultante è sufficientemente basso (di solito inferiore a 0,05), concludiamo che il modello più complesso è significativamente migliore del modello più semplice, favorendo quindi il modello più complesso. Se il p-value non è sufficientemente basso (di solito maggiore di 0,05), dovremmo privilegiare il modello più semplice. Nel nostro caso nel secondo modello come indicato da df=-9 lo stepAIC ha rimosso 9 attributi per migliorare il modello, per cui il secondo modello presenta 9 attributi in meno rispetto al primo. Nel nostro caso inoltre il p-value è sicuramente più basso rispetto a 0,05 per cui il modello nuovo con 9 attributi in meno rispetto il primo è sicuramente migliore per cui tenderemomna privilegiare il secondo modello. In sostanza se il p-value è minore del livello di significatività scelto (solitamente pari a 0,05) allora si può concludere che il modello fornisce un adattamento migliore ai dati.

La statistica F è semplicemente un rapporto di due varianze. Le varianze sono una misura della dispersione, ovvero quanto i dati sono dispersi dalla media. Valori maggiori rappresentano una dispersione maggiore. Se le osservazioni per ciascun gruppo sono vicine alla media del gruppo, la varianza all'interno dei campioni è bassa. Tuttavia, se le osservazioni per ciascun gruppo sono più lontane dalla media del gruppo, la varianza all'interno dei campioni è maggiore. Nel nostro caso abbiamo una f-test di 7 per cui non è un valore eccessivamente elevato anche se in generale, una statistica F è un rapporto di due quantità che dovrebbero essere approssimativamente uguali nell'ipotesi nulla, che produce una statistica F di circa 1.

#valutazione dell'rmse nel caso del modello stepAIC con tutte le variabili e del modello stepAIC k con 9 variabili in meno

```{r valutazione dell'rmse nel caso dei due modelli applicati}
...
R2(lm_pred2, test_set$log_selling_price)
```

# in questo caso è strano che la radice dell'errore quadratico medio sia più bassa nel primo modello dello strepAIC rispetto al secondo, infatti nel primo modello è di 0.2929, mentre nel secondo modello dello stepAIC k è di 0.2938, anche se c'è un distacco davvero piccolissimo. Inoltre sembra che ci sia un problema di overfitting del modello sia del primo che del secondo (anche se sempre con valori minimi) perchè si ha sia nel primo che nel secondo caso un valore dell'rmse più basso nel training test rispetto che nel test set suggerendo che il nostro modello di training potrebbe essere migliore il set di dati di set.

#controllo la linearità del modello dello stepAIC k perchè la regressione lineare è un modello parametrico, il che significa che per creare un'equazione, il modello segue alcune ipotesi classiche. La regressione lineare che non segue questa assunzione può essere fuorviante o semplicemente ha uno stimatore distorto. Per questa sezione, controlliamo solo il secondo modello. Il modello di regressione lineare presuppone che esista una relazione lineare tra i predittori e la risposta. Se la vera relazione è tutt'altro che lineare, allora praticamente tutte le conclusioni che traiamo dall'adattamento sono sospette. Inoltre, l'accuratezza della previsione del modello può essere notevolmente ridotta. I grafici residui sono un utile strumento grafico per identificare la non linearità. Se c'è un pattern nel grafico residuo, significa che il modello può essere ulteriormente migliorato o che non soddisfa l'ipotesi di linearità. Il grafico mostra la relazione tra i residui/errori con i valori previsti.

######## la linearità è accettata

```{r controllo linearità}
linearità<- data.frame(residual = modello_stepAIC_k$residuals, fitted = modello_stepAIC_k$fitted.values)

linearità %>% ggplot(aes(fitted, residual)) + geom_point() + geom_smooth() + geom_hline(aes(yintercept = 0)) + 
    theme(panel.grid = element_blank(), panel.background = element_blank())


```

#applico il test sulla normalità del modello applicando il test di shapiro_wilk. Si nota che i residui non seguono una distribuzione normale perchè il p-value è molto più piccolo di 0.05 e prendendo come ipotesi nulla la normalità dei residui bisogna respingerla e accettare l'ipotesi alternativa.

#####sistemare la normalità fino ad ottenere il p-value>0.05

```{r sharipo_wilk}
shapiro.test(modello_stepAIC_k$residuals)
hist(modello_stepAIC_k$residuals)
```

#applico il test dell'autocorrelazione. Gli errori standard calcolati per i coefficienti di regressione stimati o i valori adattati si basano sull'assunzione di termini di errore non correlati, ossia nessuna autocorrelazione. Se infatti c'è correlazione tra i termini di errore, allora gli errori standard stimati tenderanno a sottostimare i veri errori standard. Di conseguenza, gli intervalli di confidenza e previsione saranno più ristretti di quanto dovrebbero essere. Al momento si assume che l'ipotesi nulla sia che non c'è autocorrelazione. Qui si può notare che nel modello utilizzato per l'applicazione del modello D-W si ha un valore di 2 per cui non è presente autocorrelazione. Nei casi in cui si trovano valori molto inferiori a 2 allora siamo in presenza di autocorrelazione.

############l'autocorrelazione è accettata (non è presente)

```{r autocorrelazione}
library(car)
durbinWatsonTest(modello_stepAIC_k)


```

#applico il test dell'eteroschedasticità per valutare che le varianze dei termini di errore non sono costanti. Si possono identificare varianze non costanti negli errori dalla presenza di una forma ad imbuto nel grafico residuo, lo stesso con quello di linearità. In questo caso affinchè il test dell'eteroschedasticità sia negativo il p-value deve essere minore di 0.05 per cui in questo caso il nostro modello presenta il problema dell'eteroschedasticità e assumendo come ipotesi nulla quella della non presenza dell'eteroschedasticità rifiutaremo l'ipotesi nulla.

###########sistemare l'eteroschedasticità fino ad ottenere il p-value>0.05

```{r eteroschedasticità}
library(lmtest)
bptest(modello_stepAIC_k$model)
linearità%>% ggplot(aes(fitted, residual)) + geom_point() + theme_light() + geom_hline(aes(yintercept = 0))

```

#test della multicollinearità. Per multicollinearità si intende una correlazione tra le variabili/predittori indipendenti. Affinchè non sia presente multicollinearità tra le variabili i valori devono essere al di sotto del valore 5. Nel modello preso in considerazione per cui non vi sono problemi di multicollinearità.

########### la multicollinearità non è presente quindi accetto i valori ottenuti

```{r multicollinearità}
vif(modello_stepAIC_k)

```

#miglioriamo il modello

#controllo la correlazione dei dati per eliminare le variabili troppo correlate fra di loro ma come già visto dal test della multicollinearità non ci sono problemi per quanto riguarda le correlazioni fra le variabili

```{r controllo la correlazione}
ggcorr(modello_stepAIC$model, label = TRUE, label_size = 2.5, hjust = 1, layout.exp = 15)

ggcorr(modello_stepAIC_k$model, label = TRUE, label_size = 2, hjust = 1, layout.exp = 15)

cor(modello_stepAIC_k$model)
```
