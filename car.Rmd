...
Ciò che risulta è un modello con un valore dell'R quadro aggiustato elevato `Adjusted R-squared: 0.8599`. Il problema di questo modello però è che diversi attributi sono poco significativi quindi probabilmente il valore R alto è dovuto ad un fenomeno di overfitting. Questa cosa la verificheremo poi nella sezione successiva in cui testeremo i risultati sul test set. Inoltre alcuni degli attributi presenti sono correlati tra loro e dovremo quindi procedere a rimuoverli.
...
Cerchiamo ora di ridurre il numero di attributi necessari per aumentare l'interpretabilità e tentare di ridurre i problemi presenti. Per prima cosa andiamo a rimuovere dal dataset gli attributi che presentano problemi di multicollinearitá, facciamo riferimento alla tabella delle correlazioni della sezione "Analisi delle correlazioni". Dalla tabella si puó vedere che l'attributo `log_engine_CC` é molto correlato con `log_max_power_bhp` e decidiamo dunque di rimuoverlo.
...
-   selezionare una diversa "modalitá nulla" che abbia un maggior numero di attributi simili (es. una marca che faccia auto in una fascia di prezzo inferiore rispetto ad Audi)
...
I grafici dei residui anche in questo caso risultano abbastanza buoni. La forma anomala del Normal Q-Q indica una asimmetria della distribuzione; questo è probabilmente dovuto alla non perfetta normalità delle variabili considerate.

Nonostante il modello descriva bene i dati potrebbe essere molto migliore se ci fosse una variabile riferita allo stato dell'auto, questo infatti è uno dei parametri che maggiormente può influenzare il costo di un'auto usata.
...
###########sistemare l'eteroschedasticità accettata perchè accetta l'omoschedasticità rifiutando appunto l'eteroschedasticità
