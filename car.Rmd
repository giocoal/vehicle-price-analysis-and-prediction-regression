---
title: "Probability and statistic project: used car dataset"
...
Risultano tutti e due TRUE, quindi significa che per entrambi gli attributi le unità di misura delle osservazioni sono consistenti tra loro.
...
plot1 <- car %>% ggplot()+geom_histogram(aes(log_selling_price, ..density..), bins = 30, )+geom_density(aes(log_selling_price))+ggtitle('Selling_price con righe duplicate')+xlab('Selling Price')+ylab('Densità')

plot2 <- car_noduplicati %>% ggplot()+geom_histogram(aes(log_selling_price, ..density..), bins = 30, )+geom_density(aes(log_selling_price))+ggtitle('Selling_price senza righe ripeture')+xlab('Selling Price')+ylab('Densità')

plot3 <- ggplot(car, aes(sample = log_selling_price)) + stat_qq() + stat_qq_line(col = "red")+ggtitle('Normal Q-Q Plot con righe duplicate')+xlab('Theoretical Quantiles')+ylab('Sample Quantiles')

plot4 <- ggplot(car_noduplicati, aes(sample = log_selling_price)) + stat_qq() + stat_qq_line(col = "red")+ggtitle('Normal Q-Q Plot senza righe duplicate')+xlab('Theoretical Quantiles')+ylab('Sample Quantiles')
...
3.  Il terzo verifica l'assunzione dell'omoschedasticità. Nel nostro caso si ottiene con buona approssimazione una retta orizzontale che verifica l'assunzione.
...
Il valore di R quadro che si ottiene è praticamente identico a quello ottenuto sul `train_set` che era di `0.8552`, ciò significa che il modello è buono e predice bene i risultati.
