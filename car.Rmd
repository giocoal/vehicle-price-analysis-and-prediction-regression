...
Cerchiamo ora di ridurre il numero di attributi necessari per aumentare l'interpretabilitá e ridurre il problema dell'overfitting.
...
abline(coef(modello_minimo), col="red", lwd=3)
```
...
Dato che `Adjusted R-squared:  0.8917` é sostasnzialmente immutato rimuoviamo ancora gli ultimi 5 attributi (`makeDatsun, makeLand, makeFord, fuelCNG, makeFiat`) che sono quelli che mostrano significativitá un po' piú bassa.
...
Il valore dell'R quadro aggiustato é sceso leggermente a `Adjusted R-squared:  0.8882` ma resta comunque molto alto. Siamo rimasti quindi con 20 attributi che presentano tutti la massima significativitá.

Procediamo ora con il test F. Utilizzando ora la funzione Anova confrontiamo questo ultimo modello con quello ottenuto dal processo di stepAIC per vedere se la rimozione dei 18 attributi che abbiamo fatto é giustificata.
