...
Sono presenti 1183 righe che descrivono auto uguali, dello stesso anno, con gli stessi km e prezzo. Inoltre non puó trattarsi della stessa auto riventuta piú volte altrimenti cambierebbe il campo `owner`. Questa cosa è strana e per la maggior parte delle righe è probabilmente dovuta ad un cattivo inserimento dei dati, per questo decidiamo di rimuoverle.

```{r}
car_noduplicati <- distinct(car)

car %>% ggplot()+geom_histogram(aes(selling_price, ..density..), bins = 30, )+geom_density(aes(selling_price))+ggtitle('Istogramma del logaritmo di selling_price con righe duplicate')+xlab('Selling Price')+ylab('Densità')+scale_x_log10()

car_noduplicati %>% ggplot()+geom_histogram(aes(selling_price, ..density..), bins = 30, )+geom_density(aes(selling_price))+ggtitle('Istogramma del logaritmo di selling_price senza righe ripeture')+xlab('Selling Price')+ylab('Densità')+scale_x_log10()
...
Rimuovendo le righe duplicate si vede anche un miglioramento nella distribuzione della variabile `log_selling_price`.
...
car_dummy <- car_noduplicati[,car_nums_colnames_log]
...
car_dummy <- cbind(car_dummy,transmission = car_noduplicati$transmission)
...
Come primo passo realizzo un modello di regressione, sul `train_set` in cui considero tutte le 43 variabili
...
Ciò che risulta è un modello con un valore dell'R quadro aggiustato elevato `Adjusted R-squared: 0.8523`. Il problema di questo modello però è che molti degli attributi non sono significativi, quindi probabilmente il valore R alto è dovuto ad un fenomeno di overfitting. Questa cosa la verificheremo poi nella sezione successiva in cui testeremo i risultati sul test set.
...
1.  Linearitá delle relazioni tra variabile dipendente e le variabili indipendenti
