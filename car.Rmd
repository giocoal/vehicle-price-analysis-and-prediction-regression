...
Dalle distribuzioni si evince come, in media, il prezzo di vendita di una macchina a GPL usata risulti essere il minore. Invece, per le auto a Diesel, si ha una concentrazione di osservazioni a valori di `selling_price` maggiori e un prezzo medio maggiore rispetto a auto con altre tipologie di carburante.

Effettuiamo il test di indipendenza *Fisher's Exact Test* per verificare l'indipendenza tra `selling_price` e `fuel`:

```{r tabelle di contingenza e test chi-quadrato}
# Voglio verificare con il test chi-quadrato l'indipendenza tra le distribuzioni
# voglio vedere se ci sia dipendenza tra selling_price e il tipo di carburante
# Il test resituisce un warning perchè i valori delle celle della tabella di contingenza sono bassi
#ggplot_build(p)$data[[1]]
#table(car$log_selling_price, car$fuel)
chisq_price_fuel <- fisher.test(car$log_selling_price, car$fuel, simulate.p.value = TRUE)
chisq_price_fuel
#chisq_price_fuel$observed
```

Il valore del p-value, minore del livello di significatività (0.05), fa si che sia rifiutata l'ipotesi nulla e concludiamo che le due variabili non siano indipendenti.
...
Dalle distribuzioni si evince come, in media, il prezzo di vendita di un'automobile usata tenda a diminuire all'aumentare del numero di precedenti proprietari dell'auto (quindi a ogni passaggio di proprietà).

Effettuiamo il test di indipendenza *Fisher's Exact Test* per verificare l'indipendenza tra `selling_price` e `owner`:

```{r test di fisher esatto}
chisq_price_owner <- fisher.test(car$log_selling_price, car$owner, simulate.p.value = TRUE)
chisq_price_owner
```

Il valore del p-value, minore del livello di significatività (0.05), fa si che sia rifiutata l'ipotesi nulla e concludiamo che le due variabili non siano indipendenti.

### Distribuzione delle marche di auto (`make`) nel mercato dell'usato

```{r make boxplot}
car %>% ggplot(aes(make, log_selling_price))+geom_boxplot(coef = 3)+geom_jitter(alpha=0.05)+theme(axis.text.x = element_text(angle = 90, hjust = 1))
