...
La funzione anova prenderà gli oggetti del modello come argomenti e restituirà un ANOVA che verifica se il modello più complesso (modificato dallo stepAIC k) è significativamente migliore nell'acquisizione dei dati rispetto al modello più semplice (modello con lo stepAIC). Se il p-value risultante è sufficientemente basso (di solito inferiore a 0,05), concludiamo che il modello più complesso è significativamente migliore del modello più semplice. Se il p-value è maggiore di 0,05, dovremmo privilegiare il modello più semplice (stepAIC). Nel nostro caso inoltre il p-value è sicuramente più basso rispetto a 0,05 per cui il modello nuovo è sicuramente migliore e tenderemo a privilegiare quest'ultimo. In sostanza se il p-value è minore del livello di significatività scelto (solitamente pari a 0,05) allora si può concludere che il modello fornisce un adattamento migliore ai dati.

La statistica F è semplicemente un rapporto di due varianze. Le varianze sono una misura della dispersione, ovvero quanto i dati sono dispersi dalla media. Valori maggiori rappresentano una dispersione maggiore. Se le osservazioni per ciascun gruppo sono vicine alla media del gruppo, la varianza all'interno dei campioni è bassa. Tuttavia, se le osservazioni per ciascun gruppo sono più lontane dalla media del gruppo, la varianza all'interno dei campioni è maggiore. Nel nostro caso abbiamo una f-test di 13,8 per cui non è un valore eccessivamente elevato anche se in generale, una statistica F è un rapporto di due quantità che dovrebbero essere approssimativamente uguali nell'ipotesi nulla, che produce una statistica F di circa 1.

### Calcolo dell'RMSE per il modello stepAIC e per il modello stepAIC k 
...
Il valore RMSE (errore quadratico medio, Root Mean Squared Error) è una misura di errore assoluta in cui le deviazioni vengono elevate al quadrato per evitare che valori positivi e negativi possano annullarsi l'uno con l'altro. Con questa misura, inoltre, gli errori di valore maggiore vengono amplificati, caratteristica che può agevolare l'eliminazione dei metodi che presentano gli errori più significativi. L'RMSE è migliore del MAE (errore medio assoluto) perchè come detto poco prima l'RMSE eleva al quadrato la differenza tra i valori effettivi e i valori previsti, il che significa che la previsione con un errore maggiore sarà notevolmente penalizzata. Più basso è il valore dell'RMSE e migliore è il modello rappresentato. Nel nostro caso i valori dell'RMSE calcolati per entrambi i modelli, sia sul training test che sul test set, danno risultati praticamente identici per cui possiamo affermare che i modelli sono molto buoni nel predire il test set.
...
Per multicollinearità si intende una correlazione tra le variabili/predittori indipendenti. Affinchè non sia presente multicollinearità tra le variabili i valori devono essere al di sotto del valore 5. Nel modello preso in considerazione non vi sono valori uguali o superiori a 5 per cui non ci sono problemi di multicollinearità per cui accetto i valori ottenuti.
