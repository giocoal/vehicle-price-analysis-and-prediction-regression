...
La rimozione di quelli attributi ha portato il valore dell'R quadro aggiustato a `Adjusted R-squared:  0.8929`, cioé praticamente identico a quello iniziale. Togliendo quelle colonne abbiamo quindi alleggerito il modello senza perdere di performance.

I coefficienti nella tabella sono presentati nell'ordine in cui sono stati aggiunti nel processo di stepAIC, possiamo notare infatti che quelli verso il basso presentano una significatività non molto elevata. Dato l'ancora alto numero di attributi decidiamo di essere piú stringenti e di rimuovere quegli attiributi che non presentano una significatività molto elevata.

La rimozione andrebbe eseguita un attributo alla volta ma dato che vogliamo ridurre di molto la dimensionalitá inizieremo con togliere in blocco i 7 attributi con `significativitá > 0.05`. Gli attributi rimossi sono `makeKia, makeSkoda, ownerTest.Drive.Car, makeOpel, makeJeep, makeForce, seller_typeTrustmark.Dealer`.
