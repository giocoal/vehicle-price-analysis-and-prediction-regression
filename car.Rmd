...
#creo un nuovo dataframe con solo le variabili quantitative logaritmiche
...
Divido il nuovo dataset creato al punto precedente in due con rapporto 80 20 cosí da poter allenare il modello sulla partizione piú grande (`train_set`) e poterlo poi verificare su quello piú piccolo (`test_set`).
...
Ció che risulta sono due nuovi dataframe con 6292 e 1573 righe.
...
Realizzo un primo modello di regressione in cui considero tutte le 46 variabili
...
Ció che risulta é un modello con un valore dell'R quadro aggiustato elevato `R-squared: 0.893` probabilmente dovuto ad un fenomeno di overfitting. Il problema infatti di questo modello però è che molti degli attributi non sono significativi e alcuni di questi sono collineari. (lo verificheremo nella sezione successiva)

Dalla tabella dei coefficienti si può osservare anche che dei molti attributi dovuti alla marca dell'auto quelli che presentano la significatività maggiore sono quelli corrispondenti alle macchine più lussuose. Questa cosa è dovuta al fatto che il prezzo di queste auto si discosta molto dalla maggior parte delle altre auto a parità di motore e utilizzo.
...
Cerchiamo ora di ridurre il numero di attributi necessari per aumentare l'interpretabilità e tentare di ridurre i problemi appena

Guardando il grafico delle correlazioni della sezione precendente é facile notare come `log_age` e `log_max_power_bhp` siano i due attributi che più influenzano `log_selling_price` rispettivamente con valore di `0.74` e `0.67`. Scegliamo quindi `log_max_power_bhp` come primo attributo con cui costruire il modello.
...
Cerchiamo ora di raffinare il modello con un processo di forward selection. Per farlo ricorriamo alla funzione stepAIC dato il grande numero di attributi presenti. Sfruttiamo il `modello_minimo` appena realizzato, settiamo `direction = "forward"` e indichiamo come scope il `modello_completo`.

```{r stepAIC}
stepAIC(modello_minimo, direction = "forward", scope = formula(modello_completo))
```

Visto il grande numero di passaggi necessari nascondiamo l'output e riportiamo i risultati di seguito:

`log_selling_price ~ log_max_power_bhp + log_age + fuelPetrol + makeTata + transmission + makeToyota + makeChevrolet + km_driven + makeBMW + makeLexus + makeJaguar + makeMercedes.Benz + log_engine_CC + ownerFourth…Above.Owner + ownerThird.Owner + ownerSecond.Owner + makeMaruti + makeVolvo + makeAudi + makeDatsun + makeLand + makeFord + fuelCNG + makeFiat + seller_typeDealer + makeMahindra + makeVolkswagen + makeDaewoo + fuelLPG + makeKia + makeSkoda + ownerTest.Drive.Car + makeOpel + makeJeep + makeForce + seller_typeTrustmark.Dealer`

Il processo di stepAIC ha portato all'esclusione di 8 attributi tutti legati alla marche dei veicoli: makeNissan, makeMitsubishi, makeMG, makeRenault, makeHyundai, makeIsuzu, makeHonda, makeAshok. Verifico i parametri di questo nuovo modello.

```{r regressione lineare rispetto agli attributi individuati da stepAIC}
modello_stepAIC <- lm(formula = log_selling_price ~ log_max_power_bhp + log_age + fuelPetrol + makeTata + transmission + makeToyota + makeChevrolet + km_driven + makeBMW + makeLexus + makeJaguar + makeMercedes.Benz + log_engine_CC + ownerFourth...Above.Owner + ownerThird.Owner + ownerSecond.Owner + makeMaruti + makeVolvo + makeAudi + makeDatsun + makeLand + makeFord + fuelCNG + makeFiat + seller_typeDealer + makeMahindra + makeVolkswagen + makeDaewoo + fuelLPG + makeKia + makeSkoda + ownerTest.Drive.Car + makeOpel + makeJeep + makeForce + seller_typeTrustmark.Dealer, data = train_set)

summary(modello_stepAIC)
```

La rimozione di quelli attributi ha portato il valore dell'R quadro aggiustato a `Adjusted R-squared:  0.8929`, cioé praticamente identico a quello iniziale. Togliendo quelle colonne abbiamo quindi alleggerito il modello senza perdere di performance. Nel modello sono presenti peró ancora diversi attributi che sarebbe meglio rimuovere poiché collineari o insignificanti.

I coefficienti nella tabella sono presentati nell'ordine in cui sono stati aggiunti nel processo di stepAIC, possiamo notare infatti che quelli verso il basso presentano una significatività non molto elevata. Dato l'ancora alto numero di attributi decidiamo di essere piú stringenti e di rimuovere quegli attiributi che non presentano una significatività molto elevata. La rimozione andrebbe eseguita un attributo alla volta ma dato che vogliamo ridurre di molto la dimensionalitá inizieremo con togliere in blocco i 7 attributi con `significativitá > 0.05`. Gli attributi rimossi sono `makeKia, makeSkoda, ownerTest.Drive.Car, makeOpel, makeJeep, makeForce, seller_typeTrustmark.Dealer`.

```{r modello 1}
modello_1 <- lm(formula = log_selling_price ~ log_max_power_bhp + log_age + fuelPetrol + makeTata + transmission + makeToyota + makeChevrolet + km_driven + makeBMW + makeLexus + makeJaguar + makeMercedes.Benz + log_engine_CC + ownerFourth...Above.Owner + ownerThird.Owner + ownerSecond.Owner + makeMaruti + makeVolvo + makeAudi + makeDatsun + makeLand + makeFord + fuelCNG + makeFiat + seller_typeDealer + makeMahindra + makeVolkswagen + makeDaewoo + fuelLPG, data = train_set)

summary(modello_1)
```

Questa rimozione non ha portato sostanziali modifiche all' `Adjusted R-squared:  0.8925`, non intaccando quindi le performance del modello.

Rimuoviamo anche `makeDaewoo e fuelLPG`.

```{r modello 2}
modello_2 <- lm(formula = log_selling_price ~ log_max_power_bhp + log_age + fuelPetrol + makeTata + transmission + makeToyota + makeChevrolet + km_driven + makeBMW + makeLexus + makeJaguar + makeMercedes.Benz + log_engine_CC + ownerFourth...Above.Owner + ownerThird.Owner + ownerSecond.Owner + makeMaruti + makeVolvo + makeAudi + makeDatsun + makeLand + makeFord + fuelCNG + makeFiat + seller_typeDealer + makeMahindra + makeVolkswagen, data = train_set)

summary(modello_2)
```

L'R quandro é ancora alto `Adjusted R-squared:  0.8923`, passiamo quindi a rimuovere gli attributi con significativitá maggiore di 0.01: `transmission, seller_typeDealer, makeMahindra, makeVolkswagen`.

```{r modello 3}
modello_3 <- lm(formula = log_selling_price ~ log_max_power_bhp + log_age + fuelPetrol + makeTata + makeToyota + makeChevrolet + km_driven + makeBMW + makeLexus + makeJaguar + makeMercedes.Benz + log_engine_CC + ownerFourth...Above.Owner + ownerThird.Owner + ownerSecond.Owner + makeMaruti + makeVolvo + makeAudi + makeDatsun + makeLand + makeFord + fuelCNG + makeFiat, data = train_set)

summary(modello_3)
```

Dato che `Adjusted R-squared:  0.8917` è sostanzialmente immutato rimuoviamo ancora gli ultimi 5 attributi (`makeDatsun, makeLand, makeFord, fuelCNG, makeFiat`) che sono quelli che mostrano significatività un po' più bassa.

```{r modello 4}
modello_4 <- lm(formula = log_selling_price ~ log_max_power_bhp + log_age + fuelPetrol + makeTata + makeToyota + makeChevrolet + km_driven + makeBMW + makeLexus + makeJaguar + makeMercedes.Benz + log_engine_CC + ownerFourth...Above.Owner + ownerThird.Owner + ownerSecond.Owner + makeMaruti + makeVolvo + makeAudi, data = train_set)

summary(modello_4)
```

Il valore dell'R quadro aggiustato è sceso leggermente a `Adjusted R-squared:  0.8882` ma resta comunque molto alto. Siamo rimasti quindi con 20 attributi che presentano tutti la massima significatività.
...
Procediamo ora con il test F. Utilizzando ora la funzione Anova confrontiamo questo ultimo modello con quello ottenuto dal processo di stepAIC per vedere se la rimozione dei 18 attributi che abbiamo fatto è giustificata.

```{r confronto modello_4 con modello_stepAIC}
anova(modello_stepAIC, modello_4)
```
